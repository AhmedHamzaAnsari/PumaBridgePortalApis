...
function smtp_mailer($email_addresses, $time, $dealer_name, $db, $task_id, $dealer_id)
{
...
$email_addresses = ['ahmedhamzaansari.99@gmail.com']; // Add more email addresses as needed

smtp_mailer($email_addresses, date('Y-m-d H:i:s'), '', $db, 2393, 1392);

// Check if current time is 10:00 AM
// if (date('H:i') == '12:00') {
//     $sql_get_cartraige_no = "SELECT it.*,dl.name as dealers_name
//     FROM inspector_task as it
//     join dealers as dl on dl.id=it.dealer_id
//     WHERE it.email_status = 0 and it.status=1 and it.type='Inpection'
//     AND it.time = DATE_SUB(CURDATE(), INTERVAL 1 DAY);";
...
//             $time = $row["time"];
//             $dealers_name = $row["dealers_name"];
//             $sub = 'PUMA Visit Report of '.$dealers_name . ' ' .$time;

//             $email_addresses = ['Zeeshan.Uquaily@pumapakistan.com', 'abasit9119@gmail.com','usman.hameed@p2ptrack.com','ahmed.arif@pumapakistan.com']; // Add more email addresses as needed

//             smtp_mailer($email_addresses, date('Y-m-d H:i:s'), $sub, $db, $task_id, $dealer_id);
//         }
//     }
...
function report_detail($connect, $db, $task_id, $dealer_id)
{
    $query = "SELECT 
                ...
                (
                    SELECT id 
                    FROM inspector_task 
                    WHERE dealer_id = it.dealer_id AND id != it.id AND id < it.id and stock_variations_status=1
                    ORDER BY id DESC 
                    LIMIT 1
                ) AS last_visit_id,
                tr.dealer_sign
            ...
            WHERE 
                it.id = :task_id AND it.dealer_id = :dealer_id 
            GROUP BY 
                it.id
            ORDER BY 
                it.id DESC";
    ...
    if (count($result) > 0) {
        $first = $result[0];
        $last_dates = last_visits_dates($first['last_visit_id'], $first['visit_close_time']);
        ...
        <div class="container-fluid" >
            <div class="row">
                ...
                <div class="col-md-12">
                    Completion Date : <span id="survey_complete_time">' . htmlspecialchars($first['visit_close_time']) . '</span>
                </div>
...
function last_visits_dates($last_visit_id, $comp_date)
{
    // Initialize output variable
    $output = '';

    // Construct the URL for fetching data from the API
    $url = "http://151.106.17.246:8080/omCS-CMS-APIS/get/inspection/get_second_last_visit_recon.php?key=03201232927&id=" . $last_visit_id . "&report=inspection";
    ...
    if ($response) {
        ...
        if (is_array($result) && count($result) > 0) {
            // Get the last visit date from the API response
            $last_time = $result[0]['created_at'];

            // Parse the date-time strings into DateTime objects
            $completeTime = new DateTime($comp_date);
            $lastVisitDate = new DateTime($last_time);
    ...
    if (count($result) > 0) {
        ...
        <table class="dynamic_table" style="width:100%">
            ...
            <tr>
                ...
                <td>' . ($first ? number_format($first['monthly_target'], 2) : '---') . '</td>
                <td>' . ($second ? number_format($second['monthly_target'], 2) : '---') . '</td>
                <td>---</td>
                <td>---</td>
            </tr>
            <tr>
                <th>Actual todate (Ltr)</th>
                <td>' . ($first ? number_format($first['target_achieved'], 2) : '---') . '</td>
                <td>' . ($second ? number_format($second['target_achieved'], 2) : '---') . '</td>
                <td>---</td>
                <td>---</td>
            </tr>
            <tr>
                <th>Variance (Ltr)</th>
                <td>' . ($first ? number_format($first['difference'], 2) : '---') . '</td>
                <td>' . ($second ? number_format($second['difference'], 2) : '---') . '</td>
                <td>---</td>
                <td>---</td>
            </tr>
            <tr>
...
function get_task_wet_stock($connect, $db, $task_id, $dealer_id)
{
    ...
    if (count($result) > 0) {
        ...
        foreach ($result as $item) {
            ...
            $difference = intval($item['dip_new']) - intval($item['dip_old']);
