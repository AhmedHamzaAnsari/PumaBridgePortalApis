...
$sql_get_cartraige_no = "SELECT re.*,dl.name as dealer_name,us_tm.login as tm_email,us_rm.login as rm_email,us_grm.login as grm_email FROM reports_emailers as re
    ...
    join users as us_grm on us_grm.id=dl.zm where re.status=0 and rm_id!='';";
...
if ($count_contact > 0) {
    while ($row = mysqli_fetch_array($result_contact)) {
        ...
        AND (
            ...
            SELECT COUNT(*) 
            FROM survey_category_questions_rm
        )";
...
{
    $query = "SELECT * FROM survey_category_rm ORDER BY id ASC;";
    ...
    foreach ($result as $row) {
        ...
        $query1 = "SELECT sr.*, sq.question, rf.file AS cancel_file 
                   FROM survey_response_rm AS sr 
                   JOIN survey_category_questions_rm AS sq ON sq.id = sr.question_id
                   LEFT JOIN survey_response_files_rm rf ON (rf.question_id = sr.question_id AND rf.inspection_id = sr.inspection_id)
...
function report_detail($connect, $db, $task_id, $dealer_id)
{
    ...
    (SELECT id FROM inspector_task where dealer_id=it.dealer_id and id!=it.id and id<it.id and inspection=1 order by id desc limit 1) as last_visit_id,
    tr.dealer_sign
...
function last_visits_dates($last_visit_id, $comp_date, $current_id)
{
    ...
    $url = "http://151.106.17.246:8080/omCS-CMS-APIS/get/inspection/get_current_second_last_visit_recon.php?key=03201232927&id=" . $t_id . "&report=rm_inspection";
