...
if (isset($_GET['accesskey'])) {
    ...
    if ($user == '1') {
        $where_user = "ud.users_id != '$user'";
        $where_user_da = "da.created_by != '$user'";
    } else {
        $where_user = "ud.users_id = '$user'";
        $where_user_da = "da.created_by = '$user'";
    }
    ...
    if ($access_key_received == $access_key) {
        ...
        JOIN devicesnew as dc ON dc.id=ud.devices_id WHERE $where_user";

        $que2 = "SELECT COUNT(DISTINCT dc.name) as moving_devices FROM users_devices_new as ud 
        JOIN devicesnew as dc ON dc.id=ud.devices_id WHERE dc.speed > 0 AND dc.speed < 60 AND dc.time >= '$prev_date' AND $where_user";

        $que3 = "SELECT COUNT(DISTINCT dc.name) as stop_devices FROM users_devices_new as ud 
        JOIN devicesnew as dc ON dc.id=ud.devices_id WHERE dc.speed = 0 AND dc.ignition = 'Off' AND dc.time >= '$prev_date' AND $where_user";

        $que4 = "SELECT COUNT(DISTINCT dc.name) as nr_devices FROM users_devices_new as ud 
        JOIN devicesnew as dc ON dc.id=ud.devices_id WHERE dc.time <= '$prev_date' AND $where_user";

        $que5 = "SELECT COUNT(DISTINCT dc.name) as idle_devices FROM users_devices_new as ud 
        JOIN devicesnew as dc ON dc.id=ud.devices_id WHERE dc.speed = 0 AND dc.ignition = 'On' AND dc.time >= '$prev_date' AND $where_user";
        ...
        WHERE da.type = 'Night time violations' AND da.created_at >= '$from' AND da.created_at <= '$to' AND $where_user_da";

        $que8 = "SELECT COUNT(*) as excess_count FROM axcess_driving_alerts as da 
        JOIN devicesnew as dc ON dc.id=da.vehicle_id 
        WHERE da.created_at >= '$from' AND da.created_at <= '$to' AND $where_user_da";

        $que9 = "SELECT COUNT(dc.name) as overspeed_devices FROM users_devices_new as ud 
        join devicesnew as dc on dc.id=ud.devices_id where dc.time <='$prev_date' and $where_user and dc.speed>60";

$que10 = "SELECT 
   COUNT(dc.name) as night_voilation
FROM 
...
WHERE 
    ...
    AND da.created_by = '$user'";
        ...
        $result10 = $db->query($que10) or die("Error: " . mysqli_error($db));
        ...
        $post_data = array(
            ...
            'night_voilation' => $row10['night_voilation']
        );
