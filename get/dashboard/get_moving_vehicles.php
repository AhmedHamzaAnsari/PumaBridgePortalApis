...
if ($pass != '') {
    ...
    if ($id == '1') {
        ...
        $sql_query1 = "SELECT DISTINCT(dc.name), dc.tracker as vehicle_make, dc.time, dc.speed, dc.location as vlocation, 
            ...
            WHERE dc.speed>0 and  dc.speed < 60 and dc.time >='$prev_date'
              AND ud.users_id != '1' 
           ...
           dc.speed>0 and  dc.speed < 60 and dc.time >='$prev_date'
        ...
        join devicesnew as dc on dc.id=ud.devices_id
        join users as us on us.id=ud.users_id
        where  dc.speed>0 and  dc.speed < 60 and dc.time >='$prev_date' and ud.users_id='$id' ;";
