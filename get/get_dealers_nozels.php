...
if ($pass != '') {
    ...
    if ($pass == $access_key) {
        ...
        (SELECT id FROM dealer_reconcilation where nozle_id=dz.id order by id desc limit 1) as recon_data_id,
        (SELECT task_id FROM dealer_reconcilation where nozle_id=dz.id order by id desc limit 1) as task_id
