...
function update_sale_id($data, $order_id, $db, $type, $dealer_sap)
{
    ...
    $send_status_query = "UPDATE `order_main`
        SET
        `is_send` = '1',
        `sent_time` = '$d_date'
        WHERE id=$order_id";
    if (mysqli_query($db, $send_status_query)) {
        echo 'Send Done <br>';
    } else {
        echo 'Error updating sales number: ' . mysqli_error($db) . '<br>' . $send_status_query;
    }
    ...
    curl_setopt_array(
        $curl,
        array(
            CURLOPT_URL => 'http://151.106.17.246:8080/omCS-CMS-APIS/create/order/get_sales_order_id.php?data=' . $data . '&order_id=' . $order_id . '&type=' . $type . '&dealer_sap=' . $dealer_sap . '',
    ...
    if ($response !== false) {
        ...
        if (mysqli_query($db, $query)) {
            ...
            $salesOrder_log = "INSERT INTO `order_main_salesorder_log`
                (`order_id`,
                `salesOrderNo`,
                `created_at`,
                `created_by`)
                ...
                '$t_time',
                '');";
            if (mysqli_query($db, $salesOrder_log)) {
                echo 'Send Done <br>';
            } else {
                echo 'Error Inserting sales number: ' . mysqli_error($db) . '<br>' . $salesOrder_log;
            }
        } else {
            echo 'Error updating sales number: ' . mysqli_error($db) . '<br>' . $query;
        }
    } else {
        $outp = 'Error executing Curl request: ' . curl_error($curl);
