...
if (isset($_POST)) {
    ...
    $id=$_POST['id'];
        ...
        $oldResult = mysqli_query($db, "SELECT status FROM `survey_category_questions_rm` WHERE id = '$id'");
        $oldRow = mysqli_fetch_assoc($oldResult);
        $oldValue = $oldRow ? $oldRow['status'] : '';

    $query = "UPDATE `survey_category_questions_rm` SET `status`=$checkboxValue WHERE id='$id'";;

    mysqli_query($db, $query);
    logSystemActivity($db, $user_id, 'Updated', 'survey_category_questions_rm', $id, $oldValue, $checkboxValue);
